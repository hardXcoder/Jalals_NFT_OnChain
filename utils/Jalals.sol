 ...
 */
contract Missiles is Context, IERC20 {
    ...
    constructor(
        ...
        uint256 coinDropStartTimestamp
    ) {
        _name = name;
        _symbol = symbol;
        _decimals = 18;
    ...
    {
        require(
            IQueens(_JalalsAddress).ownerOf(tokenIndex) != address(0),
            "Owner cannot be 0 address"
        );
        require(
            tokenIndex < IQueens(_JalalsAddress).totalSupply(),
        ...
        returns (uint256)
    {
        ...
        if (lastClaimedTime == coinDropStart) {
            uint256 initialAllotment = IQueens(_JalalsAddress)
                .isMintedBeforeReveal(tokenIndex) == true
                ? INITIAL_ALLOTMENT.mul(BONUS_FACTOR)
                : INITIAL_ALLOTMENT;
     ...
     */
    function setBitQueenAddress(address jalalAddress) public {
        require(_JalalsAddress == address(0), "Already set");

        _JalalsAddress = jalalAddress;
    }
    ...
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint index = 0; index < tokenIndices.length; index++) {
            ...
            require(
                tokenIndices[index] < IQueens(_JalalsAddress).totalSupply(),
                "NFT at index has not been minted yet"
            );
            ...
            require(
                IQueens(_JalalsAddress).ownerOf(tokenIndex) == msg.sender,
        ...
        require(totalCqntClaimable != 0, "Accumulated Missile balance is 0");
